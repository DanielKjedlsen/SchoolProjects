...
int main(int argc, char* argv[]) {
    ...
    while (!stop) { // SIMULATION BEGINS - for each cycle:
        ...
        bool is_return_or_stop = is(RETURN_STOP, major_op); // 2
        bool is_reg_arithmetic = is(REG_ARITHMETIC, major_op);  // 2
        bool is_imm_arithmetic = is(IMM_ARITHMETIC, major_op); // 6
        bool is_reg_movq = is(REG_MOVQ, major_op); // 2
        bool is_imm_movq = is(IMM_MOVQ, major_op); // 6
        bool is_reg_movq_mem = is(REG_MOVQ_MEM, major_op); // 2
        bool is_imm_movq_mem = is(IMM_MOVQ_MEM, major_op); // 6
        bool is_cflow = is(CFLOW, major_op); /* note that this signal does not include return - though logically it could */ //6
        bool is_leaq2  = is(LEAQ2, major_op); // 3
        bool is_leaq3  = is(LEAQ3, major_op); // 3
        bool is_leaq6  = is(LEAQ6, major_op); // 7
        bool is_leaq7  = is(LEAQ7, major_op); // 7
        bool is_imm_cbranch = is(IMM_CBRANCH, major_op); // 10 

        // Right now, we can only execute instructions with a size of 2.
        // TODO 2021:
        // from info above determine the instruction size
        bool two_bytes = is_return_or_stop || is_reg_arithmetic || is_reg_movq || is_reg_movq_mem;
        bool three_bytes = is_leaq2 || is_leaq3;
        bool six_bytes = is_imm_arithmetic || is_imm_movq || is_imm_movq_mem || is_cflow;;
        bool seven_bytes = is_leaq6 || is_leaq7;
        bool ten_bytes = is_imm_cbranch;
        val ins_size = or(use_if(ten_bytes, from_int(10)), or(use_if(seven_bytes, from_int(7)), or(use_if(six_bytes, from_int(6)), 
            or(use_if(three_bytes, from_int(3)), use_if(two_bytes, from_int(2)))))); 
        ...
        bool is_jump = is_cflow && is(JMP, minor_op); // ADDED FOR CONVENIENCE
        ...
        bool store_bit = pick_one(3, minor_op);
        bool is_load  = !store_bit && is_mem_access; // TODO 2021: Detect when we're executing a load
        bool is_store = store_bit && is_mem_access; // TODO 2021: Detect when we're executing a store
        bool is_conditional = is_imm_cbranch || (is_cflow && !is_call && !is_jump); // TODO 2021: Detect if we are executing a conditional flow change

        // TODO 2021: Add additional control signals you may need below....
        // signals for jump, branch etc. are imlemented elsewhere in the code
        ...
        bool use_cflow_addr = is_cflow && comparator(minor_op, reg_out_a, reg_out_b);
        bool use_cbranch_addr = is_imm_cbranch && comparator(minor_op, reg_out_a, sext_imm_i);
        bool is_branch = is_jump || is_call || is_return || use_cflow_addr || use_cbranch_addr;
        val pc_next = or(use_if(is_jump, sext_imm_p), or(use_if(is_call, sext_imm_p), or(use_if(is_return, reg_out_b), 
                        or(use_if(use_cflow_addr, sext_imm_p), or(use_if(use_cbranch_addr, sext_imm_p), use_if(!is_branch, pc_incremented))))));
        ...
        val datapath_result = or(use_if(use_compute_result, compute_result),
                                 or(use_if(is_load, mem_out), (use_if(is_call, pc_incremented))));
